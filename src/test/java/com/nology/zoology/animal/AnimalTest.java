...
class AnimalTest {
    ...
    @Test
    @DisplayName("Set 1 star is successful")
    void receiveStar_OneStar_Success() {
        assertEquals( 0, target.getStars() );
        target.receiveStar(1);
        assertEquals( 1, target.getStars() );
    }

    @Test
    @DisplayName("Cannot receive more than 10 stars")
    void receiveStar_11Stars_Success() {
        assertEquals( 0, target.getStars() );
        target.receiveStar(5);
        target.receiveStar( 6);
        assertEquals( 10, target.getStars() );
    }

    @Test
    @DisplayName("Receive invalid stars throws exception")
    void receiveStar_InvalidInput_ThrowsException() {
        assertThrows( IllegalArgumentException.class,
                () -> target.receiveStar(0) );
        assertThrows( IllegalArgumentException.class,
    ...
    void feed_NoInputs_ReducesHunger() {
        target.feed();
        assertEquals(0, target.getHunger());
    }
